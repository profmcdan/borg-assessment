...
async def create_url(data: UrlCreate, repo: UrlRepository = Depends(get_bityl_repository)):
    found = await repo.get_by_original_url(clean_url(data.original_url))
    if found is not None:
        raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail="Url already exists")

    generated_url = generate_short_url(data.original_url)
    new_url: dict = {'original_url': clean_url(data.original_url),
                     'short_url': generated_url['short_url'],
                     'code': generated_url['code'],
                     'expiry_date': datetime.now() + timedelta(days=30), 'owner_id': str(uuid.uuid4())}
...
async def get_url_by_id(id: str, repo: UrlRepository = Depends(get_bityl_repository), redis=Depends(cache)):
...
@router.get("/short/{code}", response_model=UrlRead, status_code=http_status.HTTP_200_OK)
async def get_url_by_code(code: str, repo: UrlRepository = Depends(get_bityl_repository), redis=Depends(cache)):
...
async def update_url(id: str, data: UrlPatch, repo: UrlRepository = Depends(get_bityl_repository), redis=Depends(cache)):
    url = await repo.get(id=id)
    if url is None:
        raise HTTPException(status_code=http_status.HTTP_404_NOT_FOUND, detail="Url not found")
...
@router.delete("/{id}", response_model=StatusMessage, status_code=http_status.HTTP_200_OK)
async def delete_url(id: str, repo: UrlRepository = Depends(get_bityl_repository), redis=Depends(cache)):
    url = await repo.get(id=id)
    if url is None:
        raise HTTPException(status_code=http_status.HTTP_404_NOT_FOUND, detail="Url not found")
